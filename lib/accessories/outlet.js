...
module.exports = function(homebridge) {
    ...
    class FritzOutletAccessory extends FritzAccessory {
        ...
        async setOn(state, callback) {
            ...
            try {
                // Send command to Fritz!Box and wait for response
                await this.platform.fritz(state ? 'setSwitchOn' : 'setSwitchOff', this.ain);
                ...
                // Tell HomeKit the operation was successful
                callback(null);
            } catch (error) {
                this.platform.log.error(`Failed to switch ${this.name}:`, error);
                // Tell HomeKit the operation failed
                callback(error);
            }
        }
