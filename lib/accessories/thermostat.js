...
module.exports = function(homebridge) {
    ...
    async getCurrentTemperature() {
        ...
        if (this.device && this.device.present !== '1') {
            this.platform.log.warn(`[${this.name}] Cannot get temperature - device is OFFLINE`);
            throw new this.platform.api.hap.HapStatusError(this.platform.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE);
        }
    ...
    this.platform.fritz('getBatteryCharge', this.ain).then(batteryLevelFromApi => {
        ...
        service.fritzStatusLowBattery = isLow ? Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW : Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL;
        ...
        if (isCritical) {
            ...
            // Also update StatusFault when battery is critical
    ...
    update(device) {
        ...
        if (device) {
            ...
            // Update StatusFault based on offline status
            if (thermostatService) {
                thermostatService.getCharacteristic(Characteristic.StatusFault)
                    .updateValue(isOffline ? Characteristic.StatusFault.GENERAL_FAULT : Characteristic.StatusFault.NO_FAULT);
            ...
            if (isOffline) {
                this.platform.log.warn(`[${this.name}] Device is OFFLINE!`);
            }
