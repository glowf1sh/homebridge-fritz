...
class FritzAccessory {
    ...
    queryCurrentTemperature() {
        ...
        this.platform.fritz('getTemperature', this.ain).then(temperatureFromApi => {
            // Handle null response for offline devices
            if (temperatureFromApi === null || temperatureFromApi === undefined) {
                this.platform.log.debug(`[${this.name}] Device offline or not responding, keeping last temperature`);
                return; // Don't update, keep last known value
            }
