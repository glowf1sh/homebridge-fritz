...
FritzPlatform.prototype = {
    ...
    fritz: function(func) {
        ...
        if (fritz.makeRequest) {
            return fritz.makeRequest(func, args[0], args[1] || {});
        }
    ...
    setupPollingSystem: function() {
        ...
        // Wait for initial session before starting polling
        this.log("Setting up polling system - waiting for session...");
    ...
    pollBatteryStatus: function() {
        ...
        this.accessories.forEach(function(accessory) {
            ...
            // Only poll battery for devices that typically have batteries
    ...
    processDevicesByFunctionBitmask: function(devices) {
        ...
        devices.forEach(function(device) {
            ...
            // Determine device type and create/update accessory
            if (functionbitmask & 512) { // Bit 9: Schaltbare Steckdose
                self.addOrUpdateOutletAccessory(ain);
            } else if (functionbitmask & 256) { // Bit 8: Heizkörperregler
                self.addOrUpdateThermostatAccessory(ain);
            } else if (functionbitmask & 32) { // Bit 5: Temperatursensor
                // Only create standalone temperature sensor if it's not part of another device
                if (!(functionbitmask & 512) && !(functionbitmask & 256)) {
    ...
    updateAccessoryServices: function(accessory) {
        ...
        if (accessory.context.type === 'outlet') {
            ...
            if (hasTemp && !tempService && this.platform.deviceConfig(`${accessory.context.ain}.TemperatureSensor`, true)) {
                // Add temperature service
                const tempService = accessory.addService(Service.TemperatureSensor, accessory.displayName);
