...
function FritzPlatform(log, config, api) {
    ...
    // Validate intervals to prevent system overload
    const minimumInterval = 1000; // 1 second minimum
    for (const key in intervals) {
        if (typeof intervals[key] !== 'number' || intervals[key] < minimumInterval) {
            this.log.warn(`[FritzBox] Polling interval for '${key}' is invalid or too low (${intervals[key]}ms). Using default value of ${defaultIntervals[key]}ms.`);
            intervals[key] = defaultIntervals[key];
        }
    }
