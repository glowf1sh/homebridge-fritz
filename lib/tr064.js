...
class TR064Client {
    constructor(options = {}) {
        ...
        this.log.debug("TR064Client initialized with authentication details:", {
            host: this.host,
            port: this.port,
            ssl: this.ssl,
            user: this.user,
    ...
    async callAction(serviceType, actionName, params = {}) {
        try {
            ...
            if (this.user && this.password) {
                ...
                this.log.debug("TR-064 SOAP request attempt with Basic Auth:", {
                    action: actionName,
                    url: url,
                ...
                try {
                    ...
                    this.log.debug("Basic Auth successful for action:", actionName);
                } catch (error) {
                    authAttempts.push({ method: 'basic', user: this.user, error: error.message });
                    
                    if (error.response && error.response.status === 401) {
                        this.log.debug("Basic Auth failed, response headers:", error.response.headers);
                        
                        // Try with empty username
                        if (this.user !== '') {
                            this.log.debug("Trying with empty username...");
                            const configEmptyUser = {
                                ...this.axiosConfig,
                                headers: {
                                    ...this.axiosConfig.headers,
                                    'SOAPAction': `${serviceType}#${actionName}`
                            ...
                            try {
                                response = await axios.post(url, xml, configEmptyUser);
                                this.log.debug("Empty username authentication successful!");
                                authMethod = 'empty-user';
                            } catch (emptyUserError) {
                                authAttempts.push({ method: 'empty-user', user: '', error: emptyUserError.message });
                                
                                // Try with 'admin' as username
                                this.log.debug("Trying with 'admin' as username...");
                                const configAdmin = {
                                    ...this.axiosConfig,
                                    headers: {
                                        ...this.axiosConfig.headers,
                                ...
                                try {
                                    response = await axios.post(url, xml, configAdmin);
                                    this.log.debug("'admin' username authentication successful!");
                                    authMethod = 'admin-user';
                                } catch (adminError) {
                                    authAttempts.push({ method: 'admin-user', user: 'admin', error: adminError.message });
                                    
                                    // All attempts failed
                                    this.log.error("All authentication attempts failed:");
            ...
            if (error.response) {
                ...
                if (error.response.status === 401) {
                    ...
                    this.log.error("Note: Some FRITZ!Box models expect empty username or 'admin'");
