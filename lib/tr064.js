...
const { addDigestAuthInterceptor } = require('./axiosDigestInterceptor');
            ...
            if (this.user && this.password) {
                ...
                if (isTestEnvironment) {
                    ...
                    const tryDigestAuth = async (username) => {
                        try {
                            // Create a new axios instance for this auth attempt
                            const axiosInstance = axios.create(this.axiosConfig);
                            
                            // Add digest auth interceptor to this instance
                            addDigestAuthInterceptor(axiosInstance, {
                                username: username,
                                password: this.password
                            });
                            ...
                            const digestResponse = await axiosInstance.post(url, xml, {
                                headers: {
                                    ...this.axiosConfig.headers,
                                    'SOAPAction': `${serviceType}#${actionName}`
                                }
